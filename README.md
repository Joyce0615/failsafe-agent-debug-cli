...
- **Exploration quality** (`exploration.ts`) scores what an agent *reads and ranks before it edits*, independently of whether the patch worked. Coverage (precision/recall/F1 over relevant files, functions, and tests — scored separately, because reading the right file and the right function inside it are different competencies), dependency-path traversal (in-order vs. any-order, since reading both ends of a chain without the middle is not understanding it), ranking (MRR and recall@k), and efficiency (read precision, steps to first relevant read, redundant re-reads). Patch success is recorded but never folded in: the report emits a contingency table whose off-diagonal cells — `explored_well_but_failed` and `explored_poorly_but_passed` — are what tell you whether the corpus is guessable.
- **Agent failure attribution** (`agent-attribution.ts`) scores *which agent, at which step* went wrong in a long-horizon multi-agent trajectory, across `api`, `incident`, and `web_file` fixtures. Four axes are scored separately: agent, step (exact, tolerance band, and signed distance, so off-by-one and off-by-forty differ), evidence sufficiency, and recovery action. `joint_supported` is the share of correct attributions the system could also evidence — the gap against `joint_accuracy` is its lucky guesses. Every report carries two trivial controls (blame the last step; blame the first step that errored) and a `mean_truth_position`, because long-horizon trajectories usually fail near the end and an accuracy that does not beat "blame the last step" is measuring the corpus, not the system.
